...
#ifndef EXAMPLEANDTIMING_SPI_H_
#define EXAMPLEANDTIMING_SPI_H_


#include "beaglebone_blacklib/BlackSPI.h"
#include <string>
#include <iostream>
#include <iomanip>
...
void exampleAndTiming_SPI()
{
    ...
    spiTiming.startMeasure("2. BlackSPI::open()");
    bool isOpened = spiDemo.open(BlackLib::ReadWrite|BlackLib::NonBlock);
    spiTiming.endMeasure("2. BlackSPI::open()");
    ...
    spiTiming.startMeasure("3. BlackSPI::transfer(uint8_t, uint16_t)");
    readByte = spiDemo.transfer(writeByte,0);
    spiTiming.endMeasure("3. BlackSPI::transfer(uint8_t, uint16_t)");
    ...
    spiTiming.startMeasure("4. BlackSPI::transfer(uint8_t*, uint8_t*, size_t, uint16_t)");
    spiDemo.transfer(writeArr, readArr, sizeof readArr);
    spiTiming.endMeasure("4. BlackSPI::transfer(uint8_t*, uint8_t*, size_t, uint16_t)");
    ...
    std::cout << spiTiming.getAllMeasureTable();
    spiTiming.saveToFile("BlackSPI_timing_results.out");


}

#endif /* EXAMPLEANDTIMING_SPI_H_ */
